...
struct reorder_kernel_t {
    ...
    void operator()(::sycl::nd_item<1> item) const {
        ...
        for (std::size_t flattened_index = item.get_global_id(0);
            ...
            from_tensor.get_logical_index(
                    flattened_index, src_padded_index, true);
            to_tensor.get_logical_index(
                    flattened_index, dst_padded_index, true);

            bool dst_within_data_region
                    = to_tensor.within_bounds(dst_padded_index, false);
            bool dst_within_padded_region
                    = to_tensor.within_bounds(dst_padded_index, true);

            bool src_within_data_region
                    = from_tensor.within_bounds(src_padded_index, false);

            // if dst_logical_index is greater than the padded dimensions, continue
            // this would also imply that it's also outside the data region
            if (not dst_within_padded_region) { continue; }

            // if dst_logical_index is in the padded region, zero it out and continue
            if (not dst_within_data_region and dst_within_padded_region) {
                to_tensor.store_md(0, dst_padded_index, true);
                continue;
            }

            // if src is in the padded region, we ignore it
            // This is to cover cases where src has more padding that dst
            if (not src_within_data_region) { continue; }

            // and the only remaining case, both are within the data region
            if (src_within_data_region) {
                float from_value = from_tensor.load_md(src_padded_index, true);
                // apply src scale and zero points;
                // dequantized_x = scale * (quantized_x - zero_point);
                if (conf_.apply_src_zp) {
                    if (conf_.src_zp_mask > 0) {
                        dim_t idx = get_quant_param_offset(src_padded_index,
